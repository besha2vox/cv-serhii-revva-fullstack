export const education = {
  courses: [
    {
      logo: 'https://media.licdn.com/dms/image/C4D0BAQGer2PeE1Lp1Q/company-logo_100_100/0/1650366467060/ithillel_logo?e=1712188800&v=beta&t=9dvOeA2S9xHA7cBqBE48CI1TqNOdQCji5Iy78NO24v4',
      name: 'Hillel IT School',
      link: 'https://ithillel.ua/',
      studyField: 'PHP Base',
      startDate: new Date('01.18.2024'),
    },

    {
      logo: 'https://media.licdn.com/dms/image/C4D0BAQGer2PeE1Lp1Q/company-logo_100_100/0/1650366467060/ithillel_logo?e=1712188800&v=beta&t=9dvOeA2S9xHA7cBqBE48CI1TqNOdQCji5Iy78NO24v4',
      name: 'Hillel IT School',
      link: 'https://ithillel.ua/',
      studyField: 'TypeScript',
      startDate: new Date('08.28.2023'),
      endDate: new Date('11.15.2023'),
    },

    {
      logo: 'https://media.licdn.com/dms/image/C4E0BAQG3W7YQhG1mjQ/company-logo_100_100/0/1668510902073/goit_start_your_career_in_it_logo?e=1712188800&v=beta&t=gx_Cs7U2d_Hbfzcifab5VtSFeiSbV56n_ets7mCWr5E',
      name: 'GoIT',
      link: 'https://goit.global/',
      studyField: 'Fullstack Developer',
      startDate: new Date('07.01.2022'),
      endDate: new Date('05.24.2023'),
    },
  ],
  university: {
    logo: 'https://media.licdn.com/dms/image/C4E0BAQFwzDcDPcpk7A/company-logo_100_100/0/1640276225010/donetsk_national_technical_university_logo?e=1712188800&v=beta&t=FCPfuEGfV3DM5mtP-JAlJDVyCMWy0dL86ECXJcxCENY',
    name: 'Donetsk National Technical University',
    link: 'https://donntu.edu.ua/',
    degree: "Bachelor's degree",
    studyField: 'Labor protection in mining',
    startDate: new Date('09.01.2008'),
    endDate: new Date('06.31.2013'),
  },
};
